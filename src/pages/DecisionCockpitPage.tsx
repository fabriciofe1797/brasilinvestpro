...
const PRIORITY_CONFIG = {
  critical: { color: 'border-red-500/30 bg-red-500/5', icon: AlertTriangle, iconColor: 'text-red-400', badge: 'bg-red-500/20 text-red-400 border-red-500/30', labelKey: 'cockpit.priorityCritical' },
  high: { color: 'border-amber-500/30 bg-amber-500/5', icon: Zap, iconColor: 'text-amber-400', badge: 'bg-amber-500/20 text-amber-400 border-amber-500/30', labelKey: 'cockpit.priorityHigh' },
  medium: { color: 'border-blue-500/30 bg-blue-500/5', icon: Lightbulb, iconColor: 'text-blue-400', badge: 'bg-blue-500/20 text-blue-400 border-blue-500/30', labelKey: 'cockpit.priorityMedium' },
  low: { color: 'border-gray-500/30 bg-gray-500/5', icon: CheckCircle2, iconColor: 'text-gray-400', badge: 'bg-gray-500/20 text-gray-400 border-gray-500/30', labelKey: 'cockpit.priorityLow' },
};
...
export default function DecisionCockpitPage() {
  ...
  }, [portfolio, assets, transactions, settings, i18n.language]);
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">{t('cockpit.title')}</h1>
            <p className="text-gray-400 text-sm">{t('cockpit.subtitle', { month: decisions.monthLabel })}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p className="text-gray-500 text-xs uppercase font-bold">{t('cockpit.summaryActions')}</p>
            <p className="text-white font-black text-2xl">{summary.totalActions}</p>
          </div>
          <div>
            <p className="text-gray-500 text-xs uppercase font-bold">{t('cockpit.summaryCritical')}</p>
            <p className={`font-black text-2xl ${summary.criticalCount > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
              {summary.criticalCount}
            </p>
          </div>
          <div>
            <p className="text-gray-500 text-xs uppercase font-bold">{t('cockpit.summaryHigh')}</p>
            <p className={`font-black text-2xl ${summary.highCount > 0 ? 'text-amber-400' : 'text-gray-400'}`}>
              {summary.highCount}
            </p>
          </div>
          <div>
            <p className="text-gray-500 text-xs uppercase font-bold">{t('cockpit.summaryInvestment')}</p>
      ...
      {/* Top Priority */}
      {summary.totalActions > 0 && summary.topPriority && (
        <div className="bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 rounded-2xl p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-gray-400 text-xs font-bold uppercase mb-1">{t('cockpit.topPriority')}</p>
      ...
      {summary.totalActions === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-white font-bold text-xl mb-2">{t('cockpit.allClearTitle')}</h3>
          <p className="text-gray-400 text-sm max-w-md mx-auto">
            {t('cockpit.allClearDesc')}
          </p>
        </div>
      )}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5">
                  <p className="text-gray-500 text-[10px] font-bold uppercase mb-1">{t('cockpit.whyLabel')}</p>
                  <p className="text-gray-300 text-xs">{action.reason}</p>
                </div>
                <div className="bg-emerald-500/5 rounded-xl p-3 border border-emerald-500/10">
                  <p className="text-emerald-400 text-[10px] font-bold uppercase mb-1">{t('cockpit.impactLabel')}</p>
              ...
              {action.amount && action.amount > 0 && (
                <div className="mt-3 flex items-center gap-2">
                  ...
                  <span className="text-gray-400 text-xs">
                    {t('cockpit.suggestedValue')} <span className="text-white font-bold">{formatCurrency(action.amount, action.currency || 'BRL')}</span>
                  </span>
                  {action.ticker && (
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 text-center">
        <p className="text-gray-500 text-xs">
          {t('cockpit.footerNote')}
          <br />
          <span className="text-gray-600">{t('cockpit.footerUpdated', { date: new Date(decisions.generatedAt).toLocaleString(i18n.language) })}</span>
        </p>
      </div>
    </div>
  );
