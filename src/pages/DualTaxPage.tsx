...
const DualTaxPage: React.FC = () => {
  ...
  const formatMonth = (month: string) => {
    ...
    const months = t('dividends.monthsShort', { returnObjects: true }) as string[];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-[#0B1C17] border border-white/5 rounded-2xl p-6">
        <h2 className="text-sm font-bold text-white flex items-center gap-2 mb-4">
          ...
          {t('dualTax.configTitle')}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Brasil */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <p className="text-[10px] font-bold text-gray-500 uppercase mb-3">{t('dualTax.brResidence')}</p>
          ...
          {/* Portugal */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <p className="text-[10px] font-bold text-gray-500 uppercase mb-3">{t('dualTax.ptResidence')}</p>
            <div className="flex gap-2">
              <button
          ...
          {/* NHR */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <p className="text-[10px] font-bold text-gray-500 uppercase mb-3">{t('dualTax.nhrRegime')}</p>
            <div className="flex gap-2">
              <button
        ...
        <div className="mt-4 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10 flex items-start gap-3">
          ...
          <div>
            <p className="text-[10px] text-emerald-300 font-bold">{t('dualTax.treatyInfoTitle')}</p>
            <p className="text-[9px] text-gray-500 mt-1">
              {t('dualTax.treatyInfoDesc')}
              {config.nhrRegime && config.nhrType === 'old' && ' ' + t('dualTax.nhrOldNote')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#0B1C17] border border-white/5 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-bold text-gray-500 uppercase">{t('dualTax.kpiIrBr')}</span>
          </div>
          <p className="text-xl font-black text-white">{formatBRL(summary.totalBrazil)}</p>
          <p className="text-[9px] text-gray-600">{t('dualTax.kpiDarf')}</p>
        </div>
        <div className="bg-[#0B1C17] border border-white/5 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <Globe2 className="w-3.5 h-3.5 text-blue-400" />
            <span className="text-[10px] font-bold text-gray-500 uppercase">{t('dualTax.kpiIrs')}</span>
          </div>
          <p className="text-xl font-black text-white">{formatBRL(summary.totalPortugal)}</p>
          <p className="text-[9px] text-gray-600">{t('dualTax.kpiNet')}</p>
        </div>
        <div className="bg-[#0B1C17] border border-white/5 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <ArrowRightLeft className="w-3.5 h-3.5 text-purple-400" />
            <span className="text-[10px] font-bold text-gray-500 uppercase">{t('dualTax.kpiCredit')}</span>
          </div>
          <p className="text-xl font-black text-purple-400">{formatBRL(summary.totalCredits)}</p>
          <p className="text-[9px] text-gray-600">{t('dualTax.kpiSavings')}</p>
        </div>
        <div className="bg-[#0B1C17] border border-white/5 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <Shield className="w-3.5 h-3.5 text-amber-400" />
            <span className="text-[10px] font-bold text-gray-500 uppercase">{t('dualTax.kpiEffective')}</span>
          </div>
          <p className="text-xl font-black text-white">{summary.effectiveRate.toFixed(2)}%</p>
          <p className="text-[9px] text-gray-600">{t('dualTax.kpiEffectiveSub')}</p>
        </div>
      </div>
      ...
      {comparison.savings > 0 && (
        <div className="bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 rounded-2xl p-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-bold text-white">{t('dualTax.savingsTitle')}</p>
              <p className="text-[10px] text-gray-500">{t('dualTax.savingsDesc', { value: comparison.withoutTreaty.toLocaleString() })}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-2xl font-black text-emerald-400">{formatBRL(comparison.savings)}</p>
            <p className="text-[9px] text-emerald-500 font-bold">{t('dualTax.savingsBadge')}</p>
          </div>
        </div>
      )}
                ...
                <div key={cat.category} className="bg-white/5 border border-white/5 rounded-xl p-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="border-t border-white/5 pt-1.5 flex justify-between text-[10px]">
                      <span className="text-white font-bold">{t('dualTax.labelTotal')}</span>
              ...
              return (
                <div key={r.month} className="border border-white/5 rounded-xl overflow-hidden">
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {r.treaty.doubleTaxRelief > 0 && (
                          <span className="text-[9px] text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">
                            {t('dualTax.creditTag')}: {formatBRL(r.treaty.doubleTaxRelief)}
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-[9px] text-gray-600">{t('dualTax.totalBurden')}</div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                          <div className="flex items-center gap-1.5 mb-2">
                            ...
                            <span className="text-[9px] font-black text-emerald-400 uppercase">{t('dualTax.countryBr')}</span>
                          </div>
                        ...
                        <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                          <div className="flex items-center gap-1.5 mb-2">
                            ...
                            <span className="text-[9px] font-black text-blue-400 uppercase">{t('dualTax.countryPt')}</span>
                          </div>
                        ...
                        <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                          <div className="flex items-center gap-1.5 mb-2">
                            ...
                            <span className="text-[9px] font-black text-purple-400 uppercase">{t('dualTax.treatyCol')}</span>
                          </div>
                          ...
                          <div className="mt-2 pt-2 border-t border-white/5">
                            <div className="flex justify-between text-[10px]">
                              <span className="text-gray-500">{t('dualTax.totalRelief')}</span>
                              <span className="text-purple-400 font-bold">{formatBRL(r.treaty.doubleTaxRelief)}</span>
                            </div>
                            <div className="flex justify-between text-[10px]">
                              <span className="text-gray-500">{t('dualTax.netIrs')}</span>
        ...
        <div className="bg-[#0B1C17] border border-amber-500/20 rounded-2xl overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold text-white">{t('dualTax.optimizeTitle')}</h3>
            </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-amber-400 font-bold mt-1">
                        {t('dualTax.potentialSavings', { value: formatBRL(opt.savings) })}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div className="bg-[#0B1C17] border border-white/5 rounded-2xl overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-white">{t('dualTax.guideTitle')}</h3>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-white">{t('dualTax.countryBr')}</span>
            </div>
            <div className="space-y-2 text-[10px] text-gray-400">
              <p><span className="text-emerald-400 font-bold">{t('dualTax.brRuleStocks')}</span> {t('dualTax.brRuleStocksDesc')}</p>
              <p><span className="text-blue-400 font-bold">{t('dualTax.brRuleFiis')}</span> {t('dualTax.brRuleFiisDesc')}</p>
              <p><span className="text-amber-400 font-bold">{t('dualTax.brRuleFixed')}</span> {t('dualTax.brRuleFixedDesc')}</p>
              <p><span className="text-orange-400 font-bold">{t('dualTax.brRuleCrypto')}</span> {t('dualTax.brRuleCryptoDesc')}</p>
              <p><span className="text-emerald-400 font-bold">{t('dualTax.brRuleDiv')}</span> {t('dualTax.brRuleDivDesc')}</p>
            </div>
          </div>
          {/* Portugal */}
          <div className="bg-white/5 rounded-xl p-4 border border-white/5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-white">{t('dualTax.countryPt')}</span>
            </div>
            <div className="space-y-2 text-[10px] text-gray-400">
              <p><span className="text-white font-bold">{t('dualTax.ptRuleCap')}</span> {t('dualTax.ptRuleCapDesc')}</p>
              <p><span className="text-white font-bold">{t('dualTax.ptRuleDiv')}</span> {t('dualTax.ptRuleDivDesc')}</p>
              <p><span className="text-orange-400 font-bold">{t('dualTax.brRuleCrypto')}</span> {t('dualTax.ptRuleCryptoDesc')}</p>
              {config.nhrRegime && (
                <>
                  <p className="pt-2 border-t border-white/5">
                    <span className="text-purple-400 font-bold">NHR {config.nhrType === 'old' ? t('dualTax.nhrOldTag') : t('dualTax.nhrNewTag')}</span>:
                    {config.nhrType === 'old' ? ` ${t('dualTax.nhrOldDesc')}` : ` ${t('dualTax.nhrNewDesc')}`}
                  </p>
                </>
              )}
            </div>
