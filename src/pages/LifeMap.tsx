...
const CATEGORY_IDS = ['moradia', 'transporte', 'alimentacao', 'educacao', 'lazer', 'outros'] as const;
...
const LifeMap: React.FC = () => {
  ...
  const categoryLabels = t('lifeMap.categories', { returnObjects: true }) as Record<string, string>;
      ...
      <div className="relative z-10 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 pt-4">
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex items-center gap-4">
             <h1 className="text-3xl font-black tracking-tight text-white uppercase underline decoration-purple-500 decoration-4 underline-offset-8">{t('lifeMap.titleStart')}<span className="text-purple-400">{t('lifeMap.titleHighlight')}</span></h1>
             <span className="bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg shadow-purple-500/20">{t('lifeMap.badge')}</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass-card rounded-[2rem] p-8 border-white/5 relative overflow-hidden group hover:border-purple-500/20 transition-all">
             ...
             <span className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4 block">{t('lifeMap.kpiCoverage')}</span>
          ...
          <div className="glass-card rounded-[2rem] p-8 border-white/5 relative overflow-hidden group hover:border-emerald-500/20 transition-all">
             ...
             <span className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4 block">{t('lifeMap.kpiIncome')}</span>
             <div className="text-5xl font-black text-emerald-400 px-1 tracking-tighter">
                {formatCurrency(monthlyIncomeBRL, 'BRL')}
             </div>
             <div className="mt-4 text-[10px] font-black text-gray-600 uppercase tracking-widest">
                {t('lifeMap.kpiIncomeSub')}
             </div>
          </div>
          ...
          <div className="glass-card rounded-[2rem] p-8 border-white/5 relative overflow-hidden group hover:border-blue-500/20 transition-all">
             ...
             <span className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4 block">{t('lifeMap.kpiExpenses')}</span>
             <div className="text-5xl font-black text-blue-400 px-1 tracking-tighter">
                {formatCurrency(totalExpensesBRL, 'BRL')}
             </div>
             <div className="mt-4 text-[10px] font-black text-gray-600 uppercase tracking-widest">
                {t('lifeMap.kpiExpensesSub')}
             </div>
          </div>
        </div>
        ...
        <div className="glass-card rounded-[2rem] p-8 border-white/5 shadow-2xl">
          <div className="flex justify-between items-center mb-8">
             <h3 className="text-lg font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                <Target className="w-5 h-5 text-purple-500" /> {t('lifeMap.expensesTitle')}
             </h3>
             <button 
             ...
             >
               <Plus className="w-4 h-4" /> {t('lifeMap.addExpense')}
             </button>
          </div>
          ...
          {isAdding && (
            <div className="mb-6 p-6 rounded-2xl bg-purple-500/10 border border-purple-500/20">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <input 
                  type="text"
                  placeholder={t('lifeMap.namePlaceholder')}
                ...
                >
                  {CATEGORY_IDS.map(cid => (
                    <option key={cid} value={cid}>{categoryLabels[cid] ?? cid}</option>
                  ))}
                </select>
                <div className="flex gap-2">
                  <input 
                    type="number"
                    placeholder={t('lifeMap.valuePlaceholder')}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 uppercase">{categoryLabels[exp.category] ?? exp.category}</p>
                      </div>
                    </div>
                    <button 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500 uppercase mb-1">{t('lifeMap.monthlyValue')}</p>
                      <p className="text-lg font-bold text-white">
                        {formatCurrency(exp.monthlyBRL, exp.currency)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 uppercase mb-1">{t('lifeMap.coverage')}</p>
                      <p className={cn(
                        "text-lg font-bold",
          ...
          {coverageDetails.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              ...
              <p>{t('lifeMap.emptyTitle')}</p>
              <p className="text-xs mt-2">{t('lifeMap.emptySub')}</p>
            </div>
          )}
        </div>
